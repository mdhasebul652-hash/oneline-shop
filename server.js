...
function isEmployeePublicShopPath(path) {
  return path === '/' || path.startsWith('/category/') || path.startsWith('/product/') ||
    ...
    path === '/messages' || path.startsWith('/messages/') || path === '/request-inbox' || path.startsWith('/request-inbox/') ||
...
(function(){
  ...
  function saveScroll(){
    try{sessionStorage.setItem(pageKey(),JSON.stringify({y:Math.max(0,window.scrollY||window.pageYOffset||0),t:Date.now()}));}catch(e){}
  }
  function restoreScroll(){
    try{
      ...
      setTimeout(apply,100); setTimeout(apply,300); setTimeout(apply,700);
    }catch(e){}
  }
...
function bindProductControlEvents() {
  ...
  // Bind quantity controls once and support both mouse and touch/pointer input.
  if (plus && !plus.dataset.qtyBound) {
    plus.dataset.qtyBound = '1';
    const handlePlus = function(ev) {
      ev.preventDefault();
      ev.stopPropagation();
      if (!plus.disabled) incrementQty();
    };
    plus.addEventListener('click', handlePlus, {passive:false});
  }

  if (minus && !minus.dataset.qtyBound) {
    minus.dataset.qtyBound = '1';
    const handleMinus = function(ev) {
      ev.preventDefault();
      ev.stopPropagation();
      if (!minus.disabled) decrementQty();
    };
    minus.addEventListener('click', handleMinus, {passive:false});
  }
...
app.post('/admin/add-product', requireAdminSection('addProduct'), upload.fields([{ name: 'mainImage', maxCount: 1 }, { name: 'additionalImages', maxCount: 8 }]), async (req, res, next) => {
try {
if (!req.user || !isStaff(req.user) || !subAdminIsActive(req.user)) return res.redirect('/login');
...
if (!productToDelete || (!isMainAdmin(req.user) && String(productToDelete.ownerId || '') !== dataOwnerId(req.user))) return res.status(403).send('Unauthorized: Main Admin access required');

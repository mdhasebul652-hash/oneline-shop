// V79: Robust quantity controls using direct inline handlers + server fallback. Base: V78.
...
codBlockedBySellerIds: { type: [String], default: [] },
...
// TikTok OAuth / Content Posting credentials (server-side only)
tiktokClientKey: { type: String, default: '' },
tiktokClientSecret: { type: String, default: '' },
tiktokAccessToken: { type: String, default: '' },
tiktokRefreshToken: { type: String, default: '' },
tiktokOpenId: { type: String, default: '' },
tiktokScope: { type: String, default: '' },
tiktokAccessExpiresAt: { type: Date, default: null },
tiktokRefreshExpiresAt: { type: Date, default: null },
...
// V70: Dedicated Main Admin <-> Sub Admin conversation.
const adminSubAdminMessageSchema = new mongoose.Schema({
  subAdminId: { type: String, required: true, index: true },
...
const AdminSubAdminMessage = mongoose.model('AdminSubAdminMessage', adminSubAdminMessageSchema);
...
const ADMIN_SECTION_KEYS = ['addProduct','sellProducts','orders','customerManagement','qa','coupons','facebook','settings','subAdmin','productRequests','products','help','tiktok'];
const ADMIN_SECTION_LABELS = {addProduct:'Add Product',sellProducts:'Sell Products',orders:'Orders',customerManagement:'Quick Customer Management',qa:'Q&A',coupons:'Coupons',facebook:'Facebook',settings:'Settings',subAdmin:'Sub Admin Control Center',productRequests:'Product Requests',products:'All Products',help:'Main Admin Help',tiktok:'TikTok Direct Posting'};
function adminSectionLabel(key){ return ADMIN_SECTION_LABELS[String(key)] || String(key); }
...
// V72: TikTok is a first-class assignable operational section. Existing employee permissions are preserved;
// employees must have 'tiktok' in staffAssignedSections to see/use the TikTok section.
function subAdminHasSection(user, section) {
  if (isMainAdmin(user)) return true;
  const requested = String(section || '');
  const isCustomerManagement = requested === 'customerManagement' || requested === 'users';
  if (isEmployee(user)) {
    const allowed = Array.isArray(user.staffAssignedSections) ? user.staffAssignedSections : [];
    if (isCustomerManagement) return allowed.includes('customerManagement') || allowed.includes('users');
  ...
  if (requested === 'employeeManagement') return true;
  if (isCustomerManagement) return sellerOperationalSections.includes('customerManagement');
  return sellerOperationalSections.includes(requested);
}
...
function sellerProductMongoFilter(user) {
  ...
  return { ownerId: dataOwnerId(user) };
}
...
try {
  ...
  if (storeId === 'main-admin' || storeId === 'admin' || !storeId) {
    ...
    if (!mongoose.Types.ObjectId.isValid(storeId)) return res.status(404).send('Store not found');
  ...
  const logoHTML = storeLogo
    ? `<img src="${mediaUrl(storeLogo)}" alt="Store Logo" style="width:72px;height:72px;object-fit:cover;border-radius:50%;border:2px solid #f85606;background:#fff;">`
    : `<div style="width:72px;height:72px;border-radius:50%;border:2px solid #f85606;background:#fff7f2;display:flex;align-items:center;justify-content:center;font-size:34px;">🏪</div>`;
  const cards = products.map(p => `<div class="product-card" onclick="window.location.href='/product/${p._id}'" style="cursor:pointer;">
    <img src="${mediaUrl(p.mainImage)}" alt="${v46Esc(p.name||'Product')}" onclick="event.stopPropagation();openImageModal(this.src);">
    <h4 style="font-size:13px;height:38px;">${v46Esc(p.name||'Product')}</h4>
    <div class="price" style="font-size:15px;">৳${Number(p.price)||0}</div>
    <div style="font-size:11px;color:#666;">Stock: ${Number(p.stock)||0}</div>
  </div>`).join('');
  res.send(`<!DOCTYPE html><html><head><title>${v46Esc(storeName)}</title>${globalHeaderHTML}</head><body>${getNavbarHTML(req.user)}
  <div class="container">
    <div style="background:#fff;padding:18px;border-radius:10px;border:1px solid #eee;margin-bottom:15px;">
      <div style="display:flex;align-items:center;gap:14px;flex-wrap:wrap;">${logoHTML}<div style="flex:1;min-width:220px;">
        <h2 style="margin:0 0 5px 0;">${v46Esc(storeName)}</h2>
        <div style="font-size:12px;color:#666;">${owner && owner.role==='subadmin' ? 'Seller Shop' : 'Main Admin Store'}</div>
      </div></div>
    </div>
    <div style="background:#fff;padding:14px;border-radius:8px;"><h3 style="margin:0 0 12px;">🛍️ More from Store</h3>
      <div class="product-grid">${cards || '<p style="color:#777;">এই দোকানে এখন কোনো Product নেই।</p>'}</div>
    </div>
  </div></body></html>`);
} catch(err) { next(err); }
});
...
app.post('/api/product-qty/:id/:action', async (req,res,next)=>{
 try{
  const productId=String(req.params.id||''), action=String(req.params.action||''); const product=await Product.findById(productId);
  if(!product) return res.status(404).json({success:false,message:'Product not found'});
  const stock=Math.max(0,Number(product.stock)||0), maxOrder=Math.max(1,Number(product.maxOrderLimit||5)||5), max=Math.min(maxOrder,stock);
  const cookieName='productQty_'+productId.replace(/[^a-zA-Z0-9_-]/g,''); let current=Math.max(1,Math.floor(Number(req.cookies[cookieName])||1));
  if(action==='inc'&&max>0) current=Math.min(current+1,max); if(action==='dec') current=Math.max(1,current-1); if(max>0) current=Math.min(current,max);
  res.cookie(cookieName,String(current),{httpOnly:false,sameSite:'lax',maxAge:30*24*60*60*1000}); res.json({success:true,productId,quantity:current,max});
 }catch(e){next(e)}
});
...
let seller = null;
let sellerStoreId = 'main-admin';
if (product.ownerId && mongoose.Types.ObjectId.isValid(String(product.ownerId))) {
  seller = await User.findById(product.ownerId).select('name email role subAdminShopName subAdminShopLogo subAdminStatus').lean();
...
let galleryHTML = galleryData.map((item, idx) => {
  return ` <button type="button" class="thumb-btn" data-gallery-index="${idx}" aria-label="Product image ${idx+1}" aria-current="${idx===selectedGalleryIndex?'true':'false'}" onclick="return window.__osSelectGallery(${idx},event);" style="padding:0;margin:0;border:0;background:transparent;cursor:pointer;display:block;flex:0 0 auto;touch-action:manipulation;-webkit-tap-highlight-color:transparent;line-height:0;pointer-events:auto;">
    <img src="${item.src}" class="thumb-img" alt="Product image ${idx+1}" style="width:60px;height:60px;object-fit:cover;border-radius:4px;border:${idx === selectedGalleryIndex ? '2px solid #f85606' : '1px solid #ccc'};cursor:pointer;display:block;pointer-events:none;user-select:none;" draggable="false" onerror="this.style.opacity='0.35'">
  </button> `;
}).join('');
...
(function(){
  ...
  function clampQty(q){
    ...
    return maxAllowed > 0 ? Math.min(q, maxAllowed) : 1;
  }
  ...
  function renderQty(){
    currentQty = clampQty(currentQty);
    ...
    if(value) value.textContent = String(currentQty);
    if(minus){ minus.disabled = currentQty <= 1; minus.style.opacity = currentQty <= 1 ? '0.5' : '1'; }
    if(plus){ plus.disabled = maxAllowed <= 1 || currentQty >= maxAllowed; plus.style.opacity = (maxAllowed <= 1 || currentQty >= maxAllowed) ? '0.5' : '1'; }
    updateOrderLinks();
  }
  ...
  function saveQty(){
    try{
      fetch('/api/product-qty/' + encodeURIComponent(productId) + '/set?qty=' + encodeURIComponent(currentQty) + '&ajax=1', {
        method:'GET', credentials:'same-origin', cache:'no-store', keepalive:true,
        headers:{'Accept':'application/json','X-Requested-With':'XMLHttpRequest'}
      }).catch(function(){});
    }catch(_){ }
  }

  window.__osQtyChange = function(delta, ev){
    if(ev){ ev.preventDefault(); ev.stopPropagation(); }
    delta = Number(delta) > 0 ? 1 : -1;
    const before = currentQty;
    const next = clampQty(before + delta);
  ...
  window.incrementQty = function(ev){ return window.__osQtyChange(1,ev); };
  window.decrementQty = function(ev){ return window.__osQtyChange(-1,ev); };
  window.adjustProductQty = function(delta,ev){ return window.__osQtyChange(delta,ev); };
  ...
  function setMainProductImage(index, element){
    ...
    document.querySelectorAll('.thumb-img').forEach(function(t){ t.style.border='1px solid #ccc'; });
    document.querySelectorAll('.thumb-btn').forEach(function(t){ t.setAttribute('aria-current','false'); });
...
if (product) { cart[itemIndex].productName=product.name; cart[itemIndex].price=Number(product.price)||0; cart[itemIndex].deliveryCharge=Number(product.deliveryCharge)||150; cart[itemIndex].maxOrderLimit=Math.max(1,Number(product.maxOrderLimit||5)||5); cart[itemIndex].ownerId=String(product.ownerId||''); }
if (action === 'inc') {
...
window.quickCartQty = async function(ev, btn){
  ...
  if (!id || (action !== 'inc' && action !== 'dec')) return false;
  btn.disabled = true;
  const fallbackUrl = '/api/update-cart-qty/' + encodeURIComponent(id) + '/' + action;
  try {
    const r = await fetch(fallbackUrl + '?ajax=1', {method:'GET',credentials:'same-origin',cache:'no-store',headers:{'Accept':'application/json','X-Requested-With':'XMLHttpRequest'}});
    const text = await r.text(); let d=null; try{d=JSON.parse(text);}catch(_){ }
    if (!r.ok || !d || !d.success) {
    ...
    if(!item){
      const row=document.getElementById('cartItem-'+id); if(row) row.remove();
      const selectAll=document.getElementById('selectAllCart'); if(selectAll) selectAll.checked=false;
    } else {
...
app.post('/sub-admin/shop-logo', upload.single('shopLogo'), async (req,res,next)=>{
  try {
    if (!req.user || !isSubAdmin(req.user) || !subAdminIsActive(req.user)) return res.status(403).send('Seller Admin access required');
...
const controlMessageRows = await AdminSubAdminMessage.find().sort({_id:1}).limit(1000).lean();
const controlMessagesBySubAdmin = new Map();
for (const m of controlMessageRows) {
  const key = String(m.subAdminId || '');
  if (!controlMessagesBySubAdmin.has(key)) controlMessagesBySubAdmin.set(key, []);
  controlMessagesBySubAdmin.get(key).push(m);
}
...
const currentSellerIdForCustomerControl=isMainAdmin(req.user)?'':dataOwnerId(req.user);
let usersHTML = users.map(u => {
  const sellerBlocked=currentSellerIdForCustomerControl?sellerBlocksCustomer(u,currentSellerIdForCustomerControl):false;
  const activeBlock=isMainAdmin(req.user)?!!u.isBlocked:sellerBlocked;
  const statusLabel=isMainAdmin(req.user)?(u.isBlocked?'Globally Blocked (COD Off)':'Active'):(sellerBlocked?'Blocked for this Seller (COD Off)':'Active for this Seller');
  const actionLabel=isMainAdmin(req.user)?(u.isBlocked?'Unblock COD':'Block COD'):(sellerBlocked?'Unblock COD for Me':'Block COD for Me');
  return ` <div style="background:#fff; padding:8px; margin-bottom:6px; border-radius:4px; border:1px solid #eee; font-size:13px; display:flex; justify-content:space-between; align-items:center;"><div><b>${u.name || 'No Name'}</b> (${u.email})<br><span>Phone: ${u.phone || 'N/A'} | Addr: ${u.address || 'N/A'}</span></div><div><span style="color:${activeBlock ? 'red' : 'green'}; font-weight:bold; margin-right:8px;">${statusLabel}</span><a href="/admin/toggle-block-user/${u._id}" class="btn" style="background:${activeBlock ? '#28a745' : '#dc3545'}; padding:4px 8px; font-size:11px;">${actionLabel}</a></div></div> `;
}).join('');
...
const tiktokProductOptions = products.map(p => '<option value="'+String(p._id)+'">'+tiktokEsc(p.name)+' — ৳'+String(p.price)+'</option>').join('');
const tiktokConnected = !!siteSetting.tiktokAccessToken;
...
function applySectionPermissions(){ if(IS_MAIN_ADMIN)return; document.querySelectorAll('[data-section-key]').forEach(x=>{const key=x.dataset.sectionKey; const allowed=ADMIN_ALLOWED_SECTIONS.includes(key) || (key==='customerManagement' && ADMIN_ALLOWED_SECTIONS.includes('users')); if(!allowed){x.style.display='none';x.classList.add('staff-hidden');}}); const map={productRequestsBox:'productRequests',productsBox:'products',subAdminBox:'subAdmin',usersBox:'customerManagement'}; Object.entries(map).forEach(([id,key])=>{const el=document.getElementById(id);if(el&&!ADMIN_ALLOWED_SECTIONS.includes(key)){el.style.display='none';el.classList.add('staff-hidden');}}); if(document.body.classList.contains('staff-restricted-dashboard')){ document.querySelectorAll('.admin-launch-box[data-section-key]').forEach(x=>{if(!ADMIN_ALLOWED_SECTIONS.includes(x.dataset.sectionKey)){x.style.display='none';}}); const first=ADMIN_ALLOWED_SECTIONS.find(k=>k!=='employeeManagement')||ADMIN_ALLOWED_SECTIONS[0]; const firstKey=(first==='users'?'customerManagement':first); const idMap={addProduct:'addProductBox',sellProducts:'sellProductsBox',orders:'ordersBox',users:'usersBox',customerManagement:'usersBox',qa:'qaBox',coupons:'couponsBox',facebook:'facebookBox',settings:'settingsBox',productRequests:'productRequestsBox',products:'productsBox',help:'mainAdminHelpBox',subAdmin:'subAdminBox',tiktok:'tiktokBox'}; const firstId=idMap[first]; if(firstId){openAdminSection(firstId);} }}
...
${(() => { const msgs=controlMessagesBySubAdmin.get(String(sa._id))||[]; return `<div style="margin-top:10px;background:#f8fbff;border:1px solid #cfe8ff;border-radius:10px;padding:10px;"><b>💬 Admin ↔ Sub Admin Messages</b><div style="max-height:220px;overflow-y:auto;margin-top:7px;">${msgs.length?msgs.map(m=>`<div style="padding:7px 8px;margin:5px 0;border-radius:8px;background:${normalizeEmail(m.senderEmail)===normalizeEmail(req.user.email)?'#fff1e8':'#eef7ff'};border:1px solid #e8edf3;"><div style="font-size:11px;color:#777;">${m.senderRole==='subadmin'?'Sub Admin':m.senderRole==='staff'?'Employee':'Main Admin'} • ${m.senderEmail} • ${new Date(m.createdAt).toLocaleString()}</div><div style="font-size:13px;margin-top:2px;white-space:pre-wrap;">${v46Esc(m.message)}</div></div>`).join(''):'<div style="color:#777;font-size:12px;padding:8px;">এখনও কোনো message নেই।</div>'}</div><form action="/admin/subadmin/message/${sa._id}" method="POST" style="display:flex;gap:5px;margin-top:8px;"><input type="text" name="message" placeholder="এই conversation-এ message/reply লিখুন..." required maxlength="2000" style="flex:1;padding:7px;border:1px solid #ccc;border-radius:7px;"><button class="btn" style="padding:6px 10px;background:#007bff;">Reply</button></form></div>`; })()}
</details></div>`).join('')||'<div class="sub-admin-empty">কোনো active Sub Admin নেই।</div>'}
...
app.post('/sub-admin/support', async (req,res,next)=>{ try { if(!isSubAdmin(req.user)) return res.redirect('/login'); const message=String(req.body.message||'').trim(); const requestedWhatsApp=String(req.body.requestedWhatsApp||'').trim(); if(!message) return res.redirect('/admin-dashboard'); if(requestedWhatsApp && !isValidWhatsAppContact(requestedWhatsApp)) return res.send(`<script>alert('সঠিক WhatsApp Number / wa.me Link দিন।');window.history.back();</script>`); await new SubAdminSupport({subAdminId:String(req.user._id),subAdminEmail:req.user.email,message,requestedWhatsApp,whatsappUpdateStatus:requestedWhatsApp?'pending':'none'}).save();
    // Fix: AdminSubAdminMessage-এও save করো যাতে Admin Control Center-এ দেখা যায়
    const mainAdminUser = await User.findOne({role:'admin'}).select('email').lean();
    const mainAdminEmail = normalizeEmail(process.env.MAIN_ADMIN_EMAIL || process.env.ADMIN_EMAIL || (mainAdminUser ? mainAdminUser.email : ''));
    if(mainAdminEmail){
      await new AdminSubAdminMessage({subAdminId:String(req.user._id),subAdminEmail:normalizeEmail(req.user.email),senderEmail:normalizeEmail(req.user.email),senderRole:'subadmin',recipientEmail:mainAdminEmail,message:(requestedWhatsApp?message+' [Requested WhatsApp: '+requestedWhatsApp+']':message),isRead:false}).save();
      await createNotification(mainAdminEmail,'Sub Admin Help Request',message,'/admin-dashboard#subAdminSec','admin-subadmin-message');
    }
    res.redirect('/admin-dashboard'); } catch(e){next(e);} });
...
app.post('/admin/subadmin/support/:id', async (req,res,next)=>{ try { if(!hasSubAdminControl(req.user)) return res.status(403).send('Unauthorized: Sub Admin Control Center access required'); const ticket=await SubAdminSupport.findById(req.params.id); if(!ticket) return res.status(404).send('Support request not found'); const reply=safeText(req.body.reply,2000); ticket.reply=reply; ticket.updatedAt=new Date(); await ticket.save(); if(reply){ await new AdminSubAdminMessage({subAdminId:String(ticket.subAdminId),subAdminEmail:normalizeEmail(ticket.subAdminEmail),senderEmail:normalizeEmail(req.user.email),senderRole:isEmployee(req.user)?'staff':'admin',recipientEmail:normalizeEmail(ticket.subAdminEmail),message:reply,isRead:false}).save(); await createNotification(normalizeEmail(ticket.subAdminEmail),'Main Admin Reply',reply,'/admin-dashboard?section=help#mainAdminHelpBox','admin-subadmin-message'); } res.redirect('/admin-dashboard#subAdminSec'); } catch(e){next(e);} });
...
app.post('/admin/subadmin/message/:id', async (req,res,next)=>{
  try {
    ...
    await new AdminSubAdminMessage({subAdminId:String(sa._id),subAdminEmail:normalizeEmail(sa.email),senderEmail:normalizeEmail(req.user.email),senderRole:isEmployee(req.user)?'staff':'admin',recipientEmail:normalizeEmail(sa.email),message,isRead:false}).save();
    await createNotification(normalizeEmail(sa.email),'Main Admin Message',message,'/admin-dashboard?section=help#mainAdminHelpBox','admin-subadmin-message');
    res.redirect('/admin-dashboard#subAdminSec');
  } catch(e){ next(e); }
});

app.post('/sub-admin/reply-control-message', async (req,res,next)=>{
  try {
    if(!req.user || !isSubAdmin(req.user) || !subAdminIsActive(req.user)) return res.status(403).send('Active Sub Admin access required');
    const message=safeText(req.body.message,2000);
    if(!message) return res.redirect('/admin-dashboard#mainAdminHelpBox');
    // Fix: সবসময় Main Admin কে DB থেকে খুঁজে বের করো
    const recent=await AdminSubAdminMessage.find({subAdminId:String(req.user._id)}).sort({_id:-1}).limit(20).lean();
    const latestOther=recent.find(x=>normalizeEmail(x.senderEmail)!==normalizeEmail(req.user.email));
    let recipient = latestOther ? normalizeEmail(latestOther.senderEmail) : '';
    // যদি আগে কোনো message না থাকে, DB থেকে Main Admin খুঁজে নাও
    if(!recipient){
      const configuredEmail = normalizeEmail(process.env.MAIN_ADMIN_EMAIL || process.env.ADMIN_EMAIL || '');
      if(configuredEmail) {
        recipient = configuredEmail;
      } else {
        const mainAdminUser = await User.findOne({role:'admin'}).select('email').lean();
        if(mainAdminUser) recipient = normalizeEmail(mainAdminUser.email);
      }
    }
    if(!recipient) return res.status(400).send('Main Admin recipient not found. Please contact admin directly.');
    await new AdminSubAdminMessage({subAdminId:String(req.user._id),subAdminEmail:normalizeEmail(req.user.email),senderEmail:normalizeEmail(req.user.email),senderRole:'subadmin',recipientEmail:recipient,message,isRead:false}).save();
    await createNotification(recipient,'Sub Admin Reply',message,'/admin-dashboard#subAdminSec','admin-subadmin-message');
    res.redirect('/admin-dashboard#mainAdminHelpBox');
  } catch(e){ next(e); }
});
...
const TIKTOK_REDIRECT_URI = `${SITE_URL_FALLBACK.replace(/\/$/,'')}/auth/tiktok/callback`;
function tiktokSettingFor(user){ return isMainAdmin(user) ? SiteSetting.findOne() : SiteSetting.findOne({ownerId:dataOwnerId(user)}); }
function tiktokEsc(v){ return String(v||'').replace(/[&<>"']/g, c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])); }
async function refreshTikTokToken(setting){
  if(!setting || !setting.tiktokRefreshToken || !setting.tiktokClientKey || !setting.tiktokClientSecret) return false;
  const body=new URLSearchParams({client_key:setting.tiktokClientKey,client_secret:setting.tiktokClientSecret,grant_type:'refresh_token',refresh_token:setting.tiktokRefreshToken});
  const r=await fetch(TIKTOK_TOKEN_URL,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded','Cache-Control':'no-cache'},body});
  const j=await r.json();
  if(!r.ok || !j.access_token) throw new Error(j.error_description || j.error || 'TikTok token refresh failed');
  setting.tiktokAccessToken=j.access_token;
  setting.tiktokRefreshToken=j.refresh_token || setting.tiktokRefreshToken;
  setting.tiktokAccessExpiresAt=new Date(Date.now()+Number(j.expires_in||86400)*1000);
  setting.tiktokRefreshExpiresAt=new Date(Date.now()+Number(j.refresh_expires_in||31536000)*1000);
  setting.tiktokOpenId=j.open_id || setting.tiktokOpenId;
  setting.tiktokScope=j.scope || setting.tiktokScope;
  await setting.save();
  return true;
}
async function ensureTikTokToken(setting){
  if(!setting) return false;
  if(setting.tiktokAccessToken && setting.tiktokAccessExpiresAt && new Date(setting.tiktokAccessExpiresAt).getTime()>Date.now()+120000) return true;
  if(setting.tiktokRefreshToken) return await refreshTikTokToken(setting);
  return !!setting.tiktokAccessToken;
}
app.get('/auth/tiktok', requireAdminSection('tiktok'), async (req,res,next)=>{
  try{
    const setting=await tiktokSettingFor(req.user);
    if(!setting || !setting.tiktokClientKey) return res.status(400).send(`<div style="font-family:Arial;padding:25px"><h3>TikTok Client Key সেট করুন</h3><p>Admin Settings-এ TikTok Client Key ও Client Secret Save করুন।</p><a href="/admin-dashboard#tiktokSec">Back to TikTok</a></div>`);
    const state=crypto.randomBytes(24).toString('hex');
    res.cookie('tiktok_oauth_state',state,{httpOnly:true,sameSite:'lax',secure:true,maxAge:10*60*1000});
    const scope='user.info.basic,video.publish';
    const q=new URLSearchParams({client_key:setting.tiktokClientKey,response_type:'code',scope,redirect_uri:TIKTOK_REDIRECT_URI,state});
    res.redirect(TIKTOK_AUTHORIZE_URL+'?'+q.toString());
  }catch(e){next(e)}
});
app.get('/auth/tiktok/callback', async (req,res,next)=>{
  try{
    const state=String(req.query.state||'');
    const saved=String(req.cookies && req.cookies.tiktok_oauth_state || '');
    res.clearCookie('tiktok_oauth_state');
    if(!state || !saved || state!==saved) return res.status(400).send('TikTok authorization state mismatch. Please try again.');
    if(req.query.error) return res.status(400).send(`TikTok authorization failed: ${tiktokEsc(req.query.error_description||req.query.error)}`);
    if(!req.user) return res.redirect('/login?redirect=/admin-dashboard');
    const setting=await tiktokSettingFor(req.user);
    if(!setting || !setting.tiktokClientKey || !setting.tiktokClientSecret) return res.status(400).send('TikTok Client Key/Secret is missing.');
    const body=new URLSearchParams({client_key:setting.tiktokClientKey,client_secret:setting.tiktokClientSecret,code:String(req.query.code||''),grant_type:'authorization_code',redirect_uri:TIKTOK_REDIRECT_URI});
    const r=await fetch(TIKTOK_TOKEN_URL,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded','Cache-Control':'no-cache'},body});
    const j=await r.json();
    if(!r.ok || !j.access_token) return res.status(400).send(`TikTok token exchange failed: ${tiktokEsc(j.error_description||j.error||'Unknown error')}`);
    setting.tiktokAccessToken=j.access_token; setting.tiktokRefreshToken=j.refresh_token||''; setting.tiktokOpenId=j.open_id||''; setting.tiktokScope=j.scope||'';
    setting.tiktokAccessExpiresAt=new Date(Date.now()+Number(j.expires_in||86400)*1000); setting.tiktokRefreshExpiresAt=new Date(Date.now()+Number(j.refresh_expires_in||31536000)*1000);
    await setting.save();
    res.redirect('/admin-dashboard?staff=1&section=tiktok#tiktokSec');
  }catch(e){next(e)}
});
app.post('/admin/tiktok/save-config', requireAdminSection('tiktok'), async (req,res,next)=>{
  try{
    if(!req.user || !isStaff(req.user) || !subAdminIsActive(req.user)) return res.redirect('/login');
    const key=String(req.body.tiktokClientKey||'').trim();
    const secret=String(req.body.tiktokClientSecret||'').trim();
    if(!key) return res.status(400).send('<div style="font-family:Arial;padding:25px"><h3>TikTok Client Key দিন</h3><a href="/admin-dashboard#tiktokSec">Back to TikTok</a></div>');
    let setting=await tiktokSettingFor(req.user);
    if(!setting){
      setting=new SiteSetting({ownerId:dataOwnerId(req.user),tiktokClientKey:key,tiktokClientSecret:secret});
    }else{
      setting.tiktokClientKey=key;
      if(secret) setting.tiktokClientSecret=secret;
      if(!setting.ownerId) setting.ownerId=dataOwnerId(req.user);
    }
    await setting.save();
    res.redirect('/admin-dashboard?staff=1&section=tiktok#tiktokSec');
  }catch(e){next(e)}
});
app.post('/admin/tiktok/disconnect', requireAdminSection('tiktok'), async (req,res,next)=>{
  try{ const setting=await tiktokSettingFor(req.user); if(setting && setting.tiktokAccessToken && setting.tiktokClientKey && setting.tiktokClientSecret){ const body=new URLSearchParams({client_key:setting.tiktokClientKey,client_secret:setting.tiktokClientSecret,token:setting.tiktokAccessToken}); await fetch('https://open.tiktokapis.com/v2/oauth/revoke/',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded','Cache-Control':'no-cache'},body}); } if(setting){setting.tiktokAccessToken='';setting.tiktokRefreshToken='';setting.tiktokOpenId='';setting.tiktokScope='';setting.tiktokUsername='';setting.tiktokAccessExpiresAt=null;setting.tiktokRefreshExpiresAt=null;await setting.save();} res.redirect('/admin-dashboard#tiktokSec'); }catch(e){next(e)}
});
app.post('/admin/tiktok/publish', requireAdminSection('tiktok'), upload.single('mediaFile'), async (req,res,next)=>{
  try{
    const setting=await tiktokSettingFor(req.user); if(!setting || !setting.tiktokAccessToken) return res.status(400).send(`<script>alert('আগে Connect TikTok Account করুন।');location.href='/admin-dashboard#tiktokSec';</script>`);
    await ensureTikTokToken(setting);
    const info=await fetch(TIKTOK_CREATOR_INFO_URL,{method:'POST',headers:{Authorization:`Bearer ${setting.tiktokAccessToken}`,'Content-Type':'application/json'}}); const creator=await info.json();
    if(!info.ok || !creator.data) throw new Error(creator.error?.message || 'TikTok creator information পাওয়া যায়নি।');
    if(!req.file) throw new Error('Media file দিন।');
    const mediaUrlValue=await uploadBufferToCloudinary(req.file,'oneline-shop/tiktok');
    if(!mediaUrlValue) throw new Error('Media storage URL তৈরি করা যায়নি। Cloudinary settings ঠিক করুন।');
    const privacy=String(req.body.privacyLevel||'PUBLIC_TO_EVERYONE');
    const allowed=creator.data.privacy_level_options||[]; if(!allowed.includes(privacy)) throw new Error('এই TikTok account-এর জন্য নির্বাচিত privacy level অনুমোদিত নয়।');
    const productId=String(req.body.productId||'').trim(); let product=null; if(productId) product=await Product.findOne({_id:productId,...ownerFilter(req.user)}).lean();
    let caption=safeText(req.body.caption,2200); if(product){ const orderUrl=`${SITE_URL_FALLBACK.replace(/\/$/,'')}/product/${product._id}`; caption += (caption?'\n\n':'')+`Order Now: ${orderUrl}`; }
    const isPhoto=String(req.body.mediaType||'video')==='photo';
    let initBody;
    if(isPhoto){ initBody={post_info:{title:safeText(req.body.title,90),description:safeText(caption,4000),privacy_level:privacy,disable_comment:false},source_info:{source:'PULL_FROM_URL',photo_cover_index:0,photo_images:[mediaUrlValue]},post_mode:'DIRECT_POST',media_type:'PHOTO'}; }
    else { initBody={post_info:{title:safeText(caption,2200),privacy_level:privacy,disable_comment:false,disable_duet:false,disable_stitch:false},source_info:{source:'PULL_FROM_URL',video_url:mediaUrlValue}}; }
    const endpoint=isPhoto?TIKTOK_PHOTO_INIT_URL:TIKTOK_VIDEO_INIT_URL;
    const r=await fetch(endpoint,{method:'POST',headers:{Authorization:`Bearer ${setting.tiktokAccessToken}`,'Content-Type':'application/json'},body:JSON.stringify(initBody)}); const j=await r.json();
    if(!r.ok || j.error?.code!=='ok') throw new Error(j.error?.message || j.error || 'TikTok publish failed');
    res.send(`<script>alert('TikTok-এ publish request সফলভাবে পাঠানো হয়েছে। Publish ID: ${tiktokEsc(j.data?.publish_id||'')}.');location.href='/admin-dashboard#tiktokSec';</script>`);
  }catch(e){next(e)}
});
...
try{
 ...
 if(targetUser){
  if(isMainAdmin(req.user)){
   targetUser.isBlocked=!targetUser.isBlocked; await targetUser.save();
   await logActivity(req.user,`${targetUser.isBlocked?'Globally blocked':'Globally unblocked'} user ${targetUser.email}`,'User',targetUser._id);
  }else{
   const sellerId=dataOwnerId(req.user); const blocked=Array.isArray(targetUser.codBlockedBySellerIds)?targetUser.codBlockedBySellerIds.map(String):[]; const idx=blocked.indexOf(String(sellerId));
   if(idx>=0) blocked.splice(idx,1); else blocked.push(String(sellerId));
   targetUser.codBlockedBySellerIds=[...new Set(blocked.filter(Boolean))]; await targetUser.save();
   await logActivity(req.user,`${targetUser.codBlockedBySellerIds.includes(String(sellerId))?'Seller-specific COD blocked':'Seller-specific COD unblocked'} user ${targetUser.email}`,'User',targetUser._id);
  }
 }
 res.redirect('/admin-dashboard');
}catch(err){next(err)}
...
app.post('/admin/facebook/update-settings', requireAdminSection('facebook'), async (req, res, next) => {
try {
if (!req.user || !isStaff(req.user) || !subAdminIsActive(req.user)) return res.redirect('/login');
let setting = isMainAdmin(req.user) ? await SiteSetting.findOne() : await SiteSetting.findOne({ ownerId: dataOwnerId(req.user) });
if (!setting) setting = new SiteSetting({ ownerId: dataOwnerId(req.user), bkashNumber:'', nagadNumber:'' });
if (req.body.pageId !== undefined) setting.pageId = String(req.body.pageId || '').trim();
if (req.body.accessToken && String(req.body.accessToken).trim()) setting.accessToken = String(req.body.accessToken).trim();
if (!isMainAdmin(req.user)) setting.ownerId = dataOwnerId(req.user);
await setting.save();
res.redirect('/admin-dashboard?section=facebook#facebookSec');
} catch (err) { next(err); }
});

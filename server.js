...
try {
if (!req.user || !isStaff(req.user) || !subAdminIsActive(req.user)) return res.redirect('/login');
const { bkashNumber, nagadNumber, pageId, accessToken, tiktokClientKey, tiktokClientSecret } = req.body;
...
if (tiktokClientKey !== undefined) setting.tiktokClientKey = String(tiktokClientKey || '').trim();
if (tiktokClientSecret && String(tiktokClientSecret).trim()) setting.tiktokClientSecret = String(tiktokClientSecret).trim();
...
tiktokClientKey: String(tiktokClientKey || '').trim(),
tiktokClientSecret: String(tiktokClientSecret || '').trim(),
ownerId: dataOwnerId(req.user)
}).save();
}
res.redirect('/admin-dashboard');

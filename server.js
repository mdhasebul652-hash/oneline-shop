...
app.post('/sub-admin/reply-control-message', async (req,res,next)=>{
  try {
    ...
    const recipient=latestOther ? normalizeEmail(latestOther.senderEmail) : normalizeEmail(process.env.MAIN_ADMIN_EMAIL || process.env.ADMIN_EMAIL || '');
    if(!recipient) return res.status(400).send('Main Admin recipient not found');

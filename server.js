            ...
            <body>
                ...
                <div class="container">
                    ...
                    <div style="background:white; padding:20px; border-radius:6px; margin-bottom:20px;">
                        <h3 style="margin-top:0;">➕ Add New Product (with Gallery & Limit)</h3>
                        <form action="/admin/add-product" method="POST" enctype="multipart/form-data" style="display:grid; gap:10px;">
                            ...
                            <input type="number" name="price" placeholder="Price (৳)" style="padding:8px; border:1px solid #ccc; border-radius:4px;" required>
                            ...
                            <textarea name="description" placeholder="Product Description" style="padding:8px; border:1px solid #ccc; border-radius:4px; height:60px;"></textarea>
                            <label style="font-size:13px; font-weight:600;">Main Image:</label>
                            <input type="file" name="mainImage" style="padding:5px;" required>
                            <label style="font-size:13px; font-weight:600;">Gallery Images (Up to 5):</label>
                            <input type="file" name="gallery" multiple style="padding:5px;">
                            <button type="submit" class="btn" style="padding:10px;">Add Product</button>
                        </form>
                    </div>
                    
                    <div style="background:white; padding:20px; border-radius:6px; margin-bottom:20px;">
                        <h3 style="margin-top:0;">🎫 Coupon Code Management</h3>
                        <form action="/admin/add-coupon" method="POST" style="display:flex; gap:10px; margin-bottom:15px; flex-wrap:wrap;">
                            <input type="text" name="code" placeholder="Coupon Code (e.g., EID100)" style="flex:1; padding:8px; border:1px solid #ccc; border-radius:4px;" required>
                            ...
                            <button type="submit" class="btn" style="padding:8px 16px;">Create Coupon</button>
                        </form>
                        <div>${couponsHTML.length ? couponsHTML : '<p style="color:#777;">No coupons created yet.</p>'}</div>
                    </div>
                    
                    <div style="background:white; padding:20px; border-radius:6px; margin-bottom:20px;">
                        <h3 style="margin-top:0;">📢 Facebook Posts & Reels Management</h3>
                        <form action="/admin/add-fb-content" method="POST" enctype="multipart/form-data" style="display:grid; gap:10px;">
                            ...
                            <input type="file" name="mediaFile" style="padding:5px;" required>
                            <input type="text" name="productLink" placeholder="Product Link (e.g. /product/ID or /)" style="padding:8px; border:1px solid #ccc; border-radius:4px;">
                            <button type="submit" class="btn" style="padding:10px;">Publish FB Post</button>
                        </form>
                    </div>
                    
                    <div style="background:white; padding:20px; border-radius:6px; margin-bottom:20px;">
                        <h3 style="margin-top:0;">💳 bKash / Nagad Number Settings</h3>
                        <form action="/admin/update-settings" method="POST" style="display:grid; gap:10px;">
                            <label style="font-size:13px; font-weight:600;">bKash Personal Number:</label>
                            <input type="text" name="bkashNumber" value="${siteSetting.bkashNumber}" style="padding:8px; border:1px solid #ccc; border-radius:4px;" required>
                            <label style="font-size:13px; font-weight:600;">Nagad Personal Number:</label>
                            <input type="text" name="nagadNumber" value="${siteSetting.nagadNumber}" style="padding:8px; border:1px solid #ccc; border-radius:4px;" required>
                            <button type="submit" class="btn" style="padding:10px;">Update Numbers</button>
                        </form>
                    </div>
                    
                    <div style="background:white; padding:20px; border-radius:6px; margin-bottom:20px;">
                        <h3 style="margin-top:0;">🚫 User COD Restriction Control</h3>
                        <div>${usersHTML.length ? usersHTML : '<p style="color:#777;">No users found.</p>'}</div>
                    </div>
                    
                    <div style="background:white; padding:20px; border-radius:6px; margin-bottom:20px;">
                        <h3 style="margin-top:0;">💬 Customer Q&A & Product Chats (${chats.length})</h3>
                        <div>${chatsHTML.length ? chatsHTML : '<p style="color:#777;">No chats or questions yet.</p>'}</div>
                    </div>
                    
                    <div style="background:white; padding:20px; border-radius:6px;">
                        <h3 style="margin-top:0;">📦 Orders Management</h3>
                        <div style="display:flex; gap:8px; margin-bottom:15px; flex-wrap:wrap;">
                            <a href="/admin-dashboard?tab=pending" class="btn" style="background:${activeTab === 'pending' ? '#f85606' : '#ccc'}; padding:6px 12px; font-size:13px;">Pending (${pendingCount})</a>
                            <a href="/admin-dashboard?tab=confirmed" class="btn" style="background:${activeTab === 'confirmed' ? '#007bff' : '#ccc'}; padding:6px 12px; font-size:13px;">Confirmed (${confirmedCount})</a>
                            <a href="/admin-dashboard?tab=delivered" class="btn" style="background:${activeTab === 'delivered' ? '#28a745' : '#ccc'}; padding:6px 12px; font-size:13px;">Delivered (${completedCount})</a>
                            <a href="/admin-dashboard?tab=cancelled" class="btn" style="background:${activeTab === 'cancelled' ? '#dc3545' : '#ccc'}; padding:6px 12px; font-size:13px;">Cancelled (${cancelledCount})</a>
                            <a href="/admin-dashboard?tab=trash" class="btn" style="background:${activeTab === 'trash' ? '#6c757d' : '#ccc'}; padding:6px 12px; font-size:13px;">Trash (${trashCount})</a>
                        </div>
                        <div>${ordersHTML.length ? ordersHTML : '<p style="color:#777;">No orders found in this category.</p>'}</div>
                    </div>
                    
                    <div style="margin-top:20px;">
                        <a href="/logout" class="btn" style="background:#d9534f; padding:10px 20px;">Logout Admin</a>
                    </div>
                </div>
            </body>
            </html>
...
app.use((err, req, res, next) => {
    ...
    res.status(500).send(`<h3 style="color:red; text-align:center; margin-top:50px;">Internal Server Error: ${err.message}</h3>`);
});
app.listen(PORT, () => {

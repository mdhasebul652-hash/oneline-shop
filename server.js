...
let galleryHTML = allImages.map((img, idx) => ` <img src="/uploads/${img}" onclick="changeMainImage('${img}', this)" style="width:60px; height:60px; object-fit:cover; border-radius:4px; border:${idx === 0 ? '2px solid #f85606' : '1px solid #ccc'}; cursor:pointer;" class="thumb-img"> `).join('');
...
<form action="/admin/update-order-status/${o._id}" method="POST" style="margin-top:5px;">
...
if (mainImageFile) {
mainImageFilename = Date.now() + '-' + mainImageFile.originalname;
fs.writeFileSync(path.join(uploadDir, mainImageFilename), mainImageFile.buffer);
}

for (let i = 0; i < additionalImageFiles.length; i++) {
const extraName = Date.now() + '-extra-' + i + '-' + additionalImageFiles[i].originalname;
fs.writeFileSync(path.join(uploadDir, extraName), additionalImageFiles[i].buffer);
additionalImageFilenames.push(extraName);
}
await new Product({
name: req.body.name,

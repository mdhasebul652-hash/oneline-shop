...
app.get('/admin/sell-products', async (req,res,next)=>{
  try {
    ...
    const users=await User.find({role:'user'}).select('name email phone address').sort({_id:-1}).lean();
